...
const ImportacaoPage = () => {
  ...
  const startImport = async () => {
    ...
    try {
      ...
      // 1. Limpe os filtros e a busca.
      // Isso irá acionar o useEffect para recarregar os dados automaticamente.
      setFiltros({});
      setBusca("");
      // A chamada para carregarBens(1) foi REMOVIDA daqui.

      // 2. Atualize o status da importação
      setImportStatus((prev) => ({
        ...prev,
        isImporting: false,
        success: bens.length,
      ...
      // Certifique-se de parar o indicador de carregamento em caso de erro também
      setImportStatus((prev) => ({ ...prev, isImporting: false }));
    }
  };
